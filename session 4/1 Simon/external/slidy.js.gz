...
this.add_listener(document,"keydown",this.key_down);this.add_listener(document,"keypress",this.key_press);this.add_listener(window,"resize",this.resized);this.add_listener(window,"scroll",this.scrolled);this.add_listener(window,"unload",this.unloaded);this.add_listener(document,"touchstart",this.touchstart);this.add_listener(document,"touchmove",this.touchmove);this.add_listener(document,"touchend",this.touchend);this.single_slide_view();this.resized();if(this.ie7)
...
{}
var touch=e.touches[0];this.start_x=touch.pageX;this.start_y=touch.pageY;this.delta_x=this.delta_y=0;},touchmove:function(e)
{var touch=e.touches[0];this.delta_x=touch.pageX-this.start_x;this.delta_y=touch.pageY-this.start_y;},touchend:function(e)
{var delay=(new Date).getTime()-this.last_tap;var dx=this.delta_x;var dy=this.delta_y;var abs_dx=Math.abs(dx);var abs_dy=Math.abs(dy);if(delay<500&&(abs_dx>100||abs_dy>100))
{if(abs_dx>0.5*abs_dy)
{if(dx<0)
w3c_slidy.next_slide(true);else
w3c_slidy.previous_slide(true);}
else if(abs_dy>2*abs_dx)
{w3c_slidy.toggle_table_of_contents();}}},before_print:function(){this.show_all_slides();this.hide_toolbar();alert("before print");},after_print:function(){if(!this.view_all)
...
break;}}
next=node.nextSibling;node=document.body.removeChild(node);div.appendChild(node);node=next;}}},collect_slides:function(){var slides=new Array();var divs=document.body.getElementsByTagName("div");for(var i=0;i<divs.length;++i)
...
{document.title=w3c_slidy.title+" ("+(w3c_slidy.slide_number+1)+")";history.pushState(0,document.title,hash);w3c_slidy.show_slide_number();return;}
